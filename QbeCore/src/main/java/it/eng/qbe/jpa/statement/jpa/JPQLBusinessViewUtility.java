...
import it.eng.qbe.runtime.query.Operand;
...
public class JPQLBusinessViewUtility {
	...
	/**
	 * Build the string with the join conditions between the views and the entity in relation with the view
	 *
	 * @param entityAliasesMaps
	 * @param query
	 *            the query
	 * @param queryWhereClause
	 *            the where clause of the query (used for add the string WHERE or AND at the beginning of the clause)
	 *
	 * @return
	 */
	 ...
	 *
	 * @param selectFields
	 *            the list of ISelectFields from where the referred entities will be extracted
	 */
	...
	/**
	 * Visit backward the branch from the field to the root.. When it finds a view it add the join condition between the entity in the path in relation with the
	 * view
	 *
	 * @param entity
	 * @param child
	 * @param viewRelations
	 *            the set of the join condition
	 * @param entityAliases
	 * @param query
	 */
	private void buildViewsRelationsBackVistitPath(IModelField field, IModelEntity entity, IModelEntity child, Set<ViewRalationClause> viewRelations,
			Map entityAliases, Query query) {
	 ...
	 *            the query
	 */
	private void addRelationForTheView(IModelField leafField, IModelEntity parent, ModelViewEntity view, IModelEntity child,
			Set<ViewRalationClause> viewRelations, Map entityAliases, Query query) {
	...
	/**
	 * links the subentity of the view and the root entity of the selected field.. For example: With out the clause: SELECT t_0.productFamily FROM
	 * ProductProduct t_1, ProductClass t_4, ProductProductClass t_2, SalesFact1998 t_3, ProductClass t_0 WHERE
	 * t_1.compId.productClassId=t_2.compId.productClassId (t_1.compId.productClassId = t_4.productClassId) (t_3.compId.productId = t_1.compId.productId)
	 *
	 * There are no relation between t0 and t4
	 *
	 * SELECT t_0.productCategory FROM ProductProduct t_1, ProductClass t_4, ProductProductClass t_2, SalesFact1998 t_3, ProductClass t_0 WHERE
	 * t_1.compId.productClassId=t_2.compId.productClassId AND (t_1.compId.productClassId = t_4.productClassId) AND (t_4.productClassId = t_0.productClassId)
	 * *****ADDS THIS ONE****** AND (t_3.compId.productId = t_1.compId.productId)
	 *
	 *
	 *
	 * @param sourceFields
	...
	/**
	 * Takes the fields of the relation source and destination entity and builds the join condition
	 *
	 * @param sourceFields
	 *            the list of the source entity in the relation
	 * @param destFields
	 *            the list of the destination entity in the relation
	 * @param entityAliases
	 * @param query
	 * @return
	 */
	 ...
	 * @authors Alberto Ghedin (alberto.ghedin@eng.it)
	 *
	 *          A simple class that stay for a couple of non ordered strings.. Note: (ViewRalationClause(a,b)).equals(ViewRalationClause(b,a)) = true..
	 *
	 */
