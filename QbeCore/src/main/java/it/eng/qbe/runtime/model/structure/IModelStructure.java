...
/**
 * represent the structure of the data exposed by a <code>IDataSource</code>. Because a data source can be built also upon different data models (i.e.
 * CompositeQbe) an <code>IModelStructure</code> can contains multiple structure, one for each model contained in the composite data source. Each structure is
 * represented trough a collection of tree hierarchies identified by the name of the model it maps.
 *
 *
 * @authors Andrea Gioia (andrea.gioia@eng.it) Alberto Ghedin (alberto.ghedin@eng.it)
 *
 */
public interface IModelStructure extends IModelObject {
	...
	/**
	 *
	 * @return The names of all the models managed by this structure. To each model is associated an independent collection of tree hierarchies of
	 *         <code>IModelObjects</code> identified by the name of the model it maps.
	 */
