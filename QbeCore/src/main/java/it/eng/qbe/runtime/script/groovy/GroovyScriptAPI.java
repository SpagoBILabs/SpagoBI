 ...
 */
package it.eng.qbe.runtime.script.groovy;
...
public class GroovyScriptAPI {
	...
	public String getCrossNavigationLink(String... args) {
		...
		Assert.assertTrue(args.length >= 2,
				"Input arguments aren't enough: you have to specify the cross navigation text and target document's label at least.");
		String text = args[0];
		String label = args[1];
		...
		if (subobject != null)
			result.append(", 'subobject' : '" + StringEscapeUtils.escapeJavaScript(subobject) + "'");
		if (newTitle != null)
			result.append(", 'title' : '" + StringEscapeUtils.escapeJavaScript(newTitle) + "'");
		if (target != null)
			result.append(", 'target' : '" + StringEscapeUtils.escapeJavaScript(target) + "'");
