...
import it.eng.qbe.runtime.model.structure.ModelCalculatedField.Slot;
import it.eng.qbe.runtime.model.structure.ModelCalculatedField.Slot.MappedValuesPunctualDescriptor;
import it.eng.qbe.runtime.model.structure.ModelCalculatedField.Slot.MappedValuesRangeDescriptor;
...
public class AbstractSelectStatementClause extends AbstractStatementClause {
	...
	public String buildClause(Query query, Map<String, Map<String, String>> entityAliasesMaps, boolean useAliases) {
		...
		try {
			...
			entityAliases = entityAliasesMaps.get(query.getId());
			...
			logger.debug("In select clause of query [" + query.getId() + "] there are [" + calculatedFieldNumber + "] calculated fields out of ["
					+ selectFields.size() + "]");
			...
			statementFields = (Couple<String, String>[]) Array.newInstance(new Couple<String, String>("", "").getClass(), selectFields.size()
					- calculatedFieldNumber);
			index = 0;
	 ...
	 *
	 * @param alias
	 *            The alias to be encapsulated, if necessary
	 * @return
	 */
