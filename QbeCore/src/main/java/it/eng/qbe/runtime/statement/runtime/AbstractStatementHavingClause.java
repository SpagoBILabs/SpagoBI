...
package it.eng.qbe.runtime.statement.runtime;

import it.eng.qbe.jpa.statement.jpa.JPQLStatementConditionalOperators;
import it.eng.qbe.runtime.query.HavingField;
import it.eng.qbe.runtime.query.Query;
import it.eng.qbe.runtime.statement.AbstractStatementFilteringClause;
...
public abstract class AbstractStatementHavingClause extends AbstractStatementFilteringClause {
		...
		if (query.getHavingFields().size() > 0) {
			...
			for (HavingField havingField : havingFields) {
				...
				if (havingField.getLeftOperand().values[0].contains("expression")) {
					...
					havingClauseElement = buildInLineCalculatedFieldClause(havingField.getOperator(), havingField.getLeftOperand(), havingField.isPromptable(),
							havingField.getRightOperand(), query, entityAliasesMaps, conditionalOperator);
	...
	private String buildHavingClauseElement(HavingField havingField, Query query, Map entityAliasesMaps) {
		...
		try {
			...
			if (parentStatement.OPERAND_TYPE_STATIC.equalsIgnoreCase(havingField.getRightOperand().type) && havingField.isPromptable()) {
