...
public class WorkSheetDefinition extends EngineAnalysisState {
	...
	public void load(byte[] rowData) throws SpagoBIEngineException {
		...
		try {
			...
			if (encodingFormatVersion.equalsIgnoreCase(CURRENT_VERSION)) {
				...
				worksheetStateJSON = worksheetViewerStateLoader.load(str);
				logger.debug("Encoding conversion has been executed succesfully");
			}

			WorkSheetDefinition workSheetDefinition = (WorkSheetDefinition) SerializationManager.deserialize(worksheetStateJSON, "application/json",
	...
	public static Map<String, List<String>> mergeDomainValuesFilters(List<Attribute> globalFilters, List<Attribute> sheetFilters)
		...
		while (globalFiltersIt.hasNext()) {
			...
			if (sheetFilters.contains(aGlobalFilter)) { // the filter is defined globally and also on sheets
				...
				if (aGlobalFilterValues.containsAll(sheetsFilterValues)) {
					...
					throw new WrongConfigurationForFiltersOnDomainValuesException("The global filter on field " + aGlobalFilter.getAlias()
