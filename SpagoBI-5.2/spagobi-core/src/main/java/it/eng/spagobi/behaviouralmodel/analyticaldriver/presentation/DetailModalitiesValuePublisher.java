...
/**
 * Publishes the results of a detail request for a LOV value into the correct jsp page according to what contained into request. If Any errors occurred during
 * the execution of the <code>DetailModalitiesValueModule</code> class, the publisher is able to call the error page with the error message caught before and
 * put into the error handler. If the input information don't fall into any of the cases declared, another error is generated.
 */
public class DetailModalitiesValuePublisher implements PublisherDispatcherIFace {
	...
	private void notifyError(EMFErrorHandler errorHandler, String message) {
		SpagoBITracer.major(SpagoBIConstants.NAME_MODULE, "DetailModalitiesValuePublisher", "getPublisherName", message);
	...
	/**
	 * Given the request at input, gets the name of the reference publisher,driving the execution into the correct jsp page, or jsp error page, if any error
	 * occurred.
	 *
	 ...
	 *
	 * @return A string representing the name of the correct publisher, which will call the correct jsp reference.
	 */
	@Override
