...
import static it.eng.spagobi.commons.constants.SpagoBIConstants.DATE_RANGE_OPTION_QUANTITY_PREFIX;
...
public class DetailParameterModule extends AbstractHttpModule {
	...
	public void service(SourceBean request, SourceBean response) throws Exception {
		...
		SpagoBITracer.debug(AdmintoolsConstants.NAME_MODULE, "DetailParameterModule", "service",
				"begin of detail Parameter modify/visualization service with message =" + message);
	...
	private void prepareParameterDetailPage(SourceBean response, Parameter parameter, ParameterUse paruse, String selected_paruse_idStr, String modality,
		...
		List paruses = DAOFactory.getParameterUseDAO().loadParametersUseByParId(parameter.getId());
		response.setAttribute("parusesList", paruses);
		if (paruse == null) {
			if (selected_paruse_idStr == null || "".equals(selected_paruse_idStr)) {
				...
				int selected_paruse_id = Integer.parseInt(selected_paruse_idStr);
				Iterator it = paruses.iterator();
				while (it.hasNext()) {
					paruse = (ParameterUse) it.next();
					if (paruse.getUseID().equals(new Integer(selected_paruse_id)))
						break;
				}
			}
		}
	...
	private ParameterUse clone(ParameterUse paruse) {
		...
		paruseClone.setOptions(paruse.getOptions());
	...
	private void modDetailParameter(SourceBean request, String mod, SourceBean response) throws EMFUserError, SourceBeanException {
		...
		try {
			...
			if (mod.equalsIgnoreCase(ObjectsTreeConstants.DETAIL_MOD)) {
				...
				Object selectedParuseIdObj = request.getAttribute("selected_paruse_id");
	...
	private ParameterUse recoverParameterUseDetails(SourceBean request, Integer parIdInt, Integer paruseIdInt) throws NumberFormatException, EMFUserError {
		...
		if (defaultMethod == null || defaultMethod.trim().equalsIgnoreCase("none")) {
			...
			if (idLovForDefaultStr == null || idLovForDefaultStr.trim().equals(""))
				idLovForDefaultStr = "-1";
		...
		if (((String) request.getAttribute("modality")).startsWith(DATE_RANGE_TYPE)) {
			paruse.setOptions(getDateRangeOptions(request));
		}
		return paruse;
	}

	/**
	 * Retrieve the date range options from request attributes
	 *
	 * @param request
	 * @return
	 */
	private String getDateRangeOptions(SourceBean request) {
		...
		while ((type = (String) request.getAttribute(SpagoBIConstants.DATE_RANGE_OPTION_TYPE_PREFIX + curr)) != null) {
			String quantity = (String) request.getAttribute(DATE_RANGE_OPTION_QUANTITY_PREFIX + curr);
			Assert.assertTrue(DATE_RANGE_VALID_TYPES.contains(type), "type is not valid");
			Assert.assertNotNull(quantity, "quantity");
			try {
				Integer.parseInt(quantity);
			} catch (NumberFormatException e) {
				Assert.assertUnreachable("quantity must be int");
			}
			options.add(new String[] { type, quantity });
			curr += 1;
		}
		// at least 1 option: server check
		Assert.assertTrue(!options.isEmpty(), "options must be not empty");
		String res = toJson(options);
		return res;
	}

	protected static String toJson(List<String[]> dateRangeOptions) {
		Assert.assertTrue(!dateRangeOptions.isEmpty(), "options must be not empty");

		StringBuilder res = new StringBuilder("{\"options\":[");
		boolean isFirst = true;
		for (String[] opt : dateRangeOptions) {
			if (!isFirst) {
				res.append(",");
			}
			res.append("{\"");
			res.append(DATE_RANGE_TYPE_JSON);
			res.append("\":\"");
			res.append(opt[0]);
			res.append("\",\"");
			res.append(DATE_RANGE_QUANTITY_JSON);
			res.append("\":\"");
			res.append(opt[1]);
			res.append("\"}");
			isFirst = false;
	...
	private Parameter recoverParameterDetails(SourceBean request, String mod) {
		...
		if (isFunctional != null)
			...
			parameter.setIsFunctional(false);
		if (isTemporal != null)
			parameter.setIsTemporal(true);
		else
	...
	private void checkForDependancies(Integer paruseId) throws EMFUserError {
		...
		if (objectsLabels != null && objectsLabels.size() > 0) {
			HashMap params = new HashMap();
			params.put(AdmintoolsConstants.PAGE, DetailParameterModule.MODULE_PAGE);
			Vector v = new Vector();
			v.add(objectsLabels.toString());
