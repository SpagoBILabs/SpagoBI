	...
	public void doService() {
		...
		if (serviceType != null && serviceType.equalsIgnoreCase(ALARMS_LIST)) {
			...
			try {
				...
				JSONArray alarmsJSON = (JSONArray) SerializerFactory.getSerializer("application/json").serialize(alarms,
						locale);
				...
				throw new SpagoBIServiceException(SERVICE_NAME, "Exception occurred while retrieving alarms", e);
			}
			...
			try {
				if (id != null) {
					...
					if (k != null) {
						List<ThresholdValue> tresholds = tresholdDao
								.loadThresholdValuesByThresholdId(k.getThresholdId());
						logger.debug("Threshold values loaded");
						JSONArray trshJSON = (JSONArray) SerializerFactory.getSerializer("application/json")
								.serialize(tresholds, locale);
				...
				throw new SpagoBIServiceException(SERVICE_NAME, "Exception occurred while retrieving tresholds", e);
			}
