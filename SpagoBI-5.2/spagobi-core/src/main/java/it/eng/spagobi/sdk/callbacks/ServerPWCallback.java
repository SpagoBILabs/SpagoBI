...
public class ServerPWCallback implements CallbackHandler {
		...
		for (int i = 0; i < callbacks.length; i++) {
			if (callbacks[i] instanceof WSPasswordCallback) {
				...
				if (pc.getUsage() == WSPasswordCallback.DECRYPT) {
					...
					userUniqueIdentifier = authenticate(userId, password); // throws WSSecurityException.FAILED_AUTHENTICATION on failure
				} else {
				...
				mc.setProperty(WSHandlerConstants.USER, userUniqueIdentifier);
			} else {
	...
	private String authenticate(String userId, String password) throws WSSecurityException {
