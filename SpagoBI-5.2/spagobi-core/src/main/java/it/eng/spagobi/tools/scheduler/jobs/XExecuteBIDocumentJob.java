			...
			for (int documentIndex = 0; documentIndex < documentLabels.length; documentIndex++) {
				...
				while (objectParametersIterator.hasNext()) {
					...
					if (executionController.directExecution()) {
						...
						String parametersString = descriptionSuffix != null && descriptionSuffix.length() > 0 ? "and parameters [" + descriptionSuffix + "] "
								: "";

						if (suspendExecutionInUniqueMailCase == true && uniqueMailForAll == true && documentDispatcher.canDispatch(document) == false) {
							logger.info("Document [" + (documentIndex + 1) + "] with label [" + documentLabel + "] " + parametersString
							...
							logger.debug("No valid dispatch target for document [" + (documentIndex + 1) + "] with label [" + documentLabel + "] "
									+ parametersString);
							logger.warn("Document [" + (documentIndex + 1) + "] with label [" + documentLabel + "] " + parametersString
									+ "will be executed but not dispatched");
							// continue;
						} else {
							logger.debug("There is at list one dispatch target for document with label [" + documentLabel + "]");
						}

						// execute document
						executionProxy = new ExecutionProxy();
						executionProxy.setBiObject(document);
						logger.info("Executing document [" + (documentIndex + 1) + "] with label [" + documentLabel + "] " + parametersString + " ...");
						...
						logger.info("Document [" + (documentIndex + 1) + "] with label [" + documentLabel + "] " + parametersString + "executed in [" + elapsed
								+ "]");
