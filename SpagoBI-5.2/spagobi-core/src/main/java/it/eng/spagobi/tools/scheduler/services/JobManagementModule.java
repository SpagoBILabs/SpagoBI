...
public class JobManagementModule extends AbstractHttpModule {
	...
	private void saveJob(JobInfo jobInfo) throws EMFUserError {
		...
		try {
			...
			while (iterbiobj.hasNext()) {
				...
				StringBuilder dateRangePeriod = new StringBuilder();
				while (iterPars.hasNext()) {
					...
					// mange darate range driver
					if (DateRangeDAOUtilities.isDateRange(biobjpar)) {
						String period = jobInfo.getDateRangePeriod(biobj, biobjpar);
						Assert.assertNotNull(period, "period is null");
						dateRangePeriod.append(biobjpar.getParameterUrlName() + "=" + period + ";");
					}
				...
				if (useFormulaParameters.length() > 0) {
					...
					message.append(
							"<PARAMETER name=\"" + biobj.getLabel() + "__" + index + "_useFormula\" value=\"" + useFormulaParameters.toString() + "\" />");
				}

				// manage date range
				if (dateRangePeriod.length() != 0) {
					// only the period, start date is treated like a value
					// delete last ";"
					dateRangePeriod.deleteCharAt(dateRangePeriod.length() - 1);
					message.append("<PARAMETER name=\"" + biobj.getLabel() + "__" + index + DateRangeDAOUtilities.DATE_RANGE_PARAMETER_SUFFIX + "\" value=\""
							+ dateRangePeriod.toString() + "\" />");
				}
	...
	private void getDocParValuesFromRequest(SourceBean request, JobInfo jobInfo) throws Exception {
		...
		while (iterbiobjs.hasNext()) {
			...
			while (iterbiobjpars.hasNext()) {
				...
				if (DateRangeDAOUtilities.isDateRange(biobjpar)) {
					manageDateRange(biobjpar, jobInfo, request, nameParInRequest, biobj);
				}
	...
	/**
	 * From: <select name='<%="par_"+biobj.getId()+"_"+index+"_"+biobjpar.getParameterUrlName()+"_period"%>' Value:
	 * <option value='<%=quantity %>_<%=type %>' > Es.: 6_years
	 *
	 * @param biobjpar
	 * @param jobInfo
	 * @param request
	 * @param nameParInRequest
	 * @param doc
	 */
	private static void manageDateRange(BIObjectParameter biobjpar, JobInfo jobInfo, SourceBean request, String nameParInRequest, BIObject doc) {
		String value = (String) request.getAttribute(nameParInRequest + "_period");
		Assert.assertTrue(value != null & !value.isEmpty(), "period not present");
		jobInfo.putDateRangePeriod(doc, biobjpar, value);
	}
