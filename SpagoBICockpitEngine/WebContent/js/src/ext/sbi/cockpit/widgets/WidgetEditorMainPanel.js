...
Sbi.cockpit.widgets.WidgetEditorMainPanel = function(config) { 

	var defaultSettings = {
		emptyMsg: LN('Sbi.cockpit.widgets.widgeteditormainpanel.emptymsg'),
		style:'padding:5px 15px 2px'
	};
		
	if(Sbi.settings && Sbi.cockpit && Sbi.cockpit.widgets && Sbi.cockpit.widgets.widgetEditorMainPanel) {
		defaultSettings = Ext.apply(defaultSettings, Sbi.cockpit.widgets.widgetEditorMainPanel);
	}
	...
	Sbi.cockpit.widgets.WidgetEditorMainPanel.superclass.constructor.call(this, c);
...
Ext.extend(Sbi.cockpit.widgets.WidgetEditorMainPanel, Ext.Panel, {
	...
	, createDummyDesigner: function(msg) {
		...
		return new Ext.Panel(Ext.apply({
			...
			, tools:  [{
		        ...
		        , scope: this
		        , qtip: LN('Sbi.cockpit.widgets.widgeteditormainpanel.tools.tt.remove')
			}]
		},sheredConf));
	}
