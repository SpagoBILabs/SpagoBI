 ...
 * 
 *         This Class encapsulates the crossTab The publics methods are: -
 *         CrossTab(IDataStore dataStore, CrosstabDefinition crosstabDefinition)
 *         that builds the crossTab (headers structure and data) -
 *         getJSONCrossTab() that returns the JSON representation of the
 *         crosstab
 */
...
public class CrossTab {
	 ...
	 *            : true if the measures live in the columns, false if the
	 *            measures live in the rows
	...
	/**
	 * Get a list of nodes and merge them.. It builds a tree with all the node
	 * in common with the subtree in with radix in input.. For example
	 * nodes=[A,[1,2,3], A,[1,6]] the result is [A,[1]] In the leafs (in this
	 * case 1) it add the position of that header in the matrix.. In this case,
	 * suppose the id of the first occurence of A.1 is at row 3 and the second
	 * in row 7 the leafs with id A.1 has this list [3,7]
	 * 
	 * @param nodes
	 * @param NodeValue
	 * @return
	 ...
	 * @param operation
	 * @param node
	 *            the result of the calculated fields will add as child of this
	 *            node
	 * @param horizontal
	 * @param level
	 * @param cfName
	...
	/**
	 * Build the list of the nodes involved in the operation and the map of the
	 * indexes es: nodes: [A,[1,2]], [B[1,3]] , [C[1,3]] operationExpsNames: [A,
	 * C] nodeInvolvedInTheOperation: [A,[1,2]], [B[1,3]] expressionToIndexMap:
	 * 0,2
	 * 
	 * @param nodes
	 ...
	 *            if the operation is the same of before and the Nodes of the
	 *            level are A,B,C,D the map is (A->0, B->1...)
	 ...
	 * 
	 * @param data
	 *            list of rows/columns members of the operation l'operazione es
	 *            [1,2], [3,4]
	 * @param operation
	 *            the operation es: [+]
	 * @return
	...
	public void addSubtotals() {
		...
		// addSubtotalsToTheNodeFirstLevel(childOfRoot.get(i), true, 2,
		// measuresOnRow);
		// }
		// }
		...
		// for(int i=0; i<childOfRoot.size(); i++){
		// addSubtotalsToTheNodeFirstLevel(childOfRoot.get(i), false, 2,
		// measuresOnRow);
		// }
		// }
	}
	...
	// * @param measuresOnRow
	// */
	// public void addSubtotalsToTheNode(Node n, boolean horizontal, int level,
	// boolean measuresOnRow){
	// List<Node> childs = n.getChilds();
	// if(measuresOnRow){
	// if((!horizontal && level<2) || //if level == 1 the subtotals are equals
	// to the total
	// (childs.size()>0 && childs.get(0).getChilds().size()==0 ) ){ //don't
	// calculate the subtotals between neasures
	// return;
	// }
	// }else{
	// if(horizontal && level<2 || (childs.size()>0 &&
	// childs.get(0).getChilds().size()==0 )){
	// return;
	// }
	// }
	// if(n.getCellType()!=CellType.CF && childs.size()>0 && n.getValue()!=TOTAL
	// && n.getValue()!=SUBTOTAL ){
	// //build the calcuated field for the sum
	// StringBuilder sb = new StringBuilder(" ");
	// for(int i=0; i<childs.size(); i++){
	// if(childs.get(i).getCellType()!=CellType.CF &&
	// childs.get(i).getValue()!=TOTAL && childs.get(i).getValue()!=SUBTOTAL ){
	// sb.append("field[");
	...
	// calculateCF(sb.toString(), n, horizontal, level, SUBTOTAL,
	// CellType.SUBTOTAL);
	// for(int i=0; i<childs.size(); i++){
	...
	// * @param measuresOnRow
	// */
	// public void addSubtotalsToTheNodeFirstLevel(Node n, boolean horizontal,
	// int level, boolean measuresOnRow){
	// List<Node> childs = n.getChilds();
	//
	//
	//
