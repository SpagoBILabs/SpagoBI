...
public class CreateExportFileAction extends AbstractConsoleEngineAction {
	...
	public void service(SourceBean request, SourceBean response) {
		...
		try {
			...
			Assert.assertNotNull(getConsoleEngineInstance().getDataSetServiceProxy(), "It's not possible to execute " + this.getActionName()
			...
			if (dataSetHeadersLabel != null && !dataSetHeadersLabel.equals("")) {
				...
				Assert.assertNotNull(dataStoreHeaders, "The dataStore returned by loadData method of the class [" + dataSetHeaders.getClass().getName()
						+ "] cannot be null");
			}
			...
			String dsQueryScript = jsonConf.getString(QUERY_SCRIPT);
			String dsQueryscriptLanguage = jsonConf.getString(QUERY_SCRIPT_LANGUAGE);
			Vector extractedFields = dsu.readFields(dsQuery, dsQueryScript, dsQueryscriptLanguage, dataSet);

			if (extractedFields != null) {
				logger.debug("CreateExportFileAction: Extracted Fields vector has dimension = " + extractedFields.size());
			}
			...
			if (jsonArray != null && jsonArray.length() > 0) {
				...
				/*
				 * for(int k = 0; k < jsonArray.length(); k++){ JSONObject resultHeaders = jsonArray.getJSONObject(k); Iterator it = resultHeaders.keys();
				 * while(it.hasNext()) { String key = (String)it.next(); JSONObject header = resultHeaders.getJSONObject(key); String fieldHeader =
				 * header.optString("header", ""); String fieldHeaderType = header.optString("headerType", ""); // // in case of dynamic headers gets the value
				 * from the dataset (of data) if (fieldHeaderType.equalsIgnoreCase("dataset")){ int posHeader =
				 * dataStore.getMetaData().getFieldIndex(fieldHeader); int fieldValsize = ((List)dataStore.getFieldValues(posHeader)).size(); if(fieldValsize !=
				 * 0){ fieldHeader =((List)dataStore.getFieldValues(posHeader)).get(0).toString(); }else{ fieldHeader = null; } }else if
				 * (fieldHeaderType.equalsIgnoreCase("datasetI18N") && dataStoreHeaders != null){ //gets the header value from the specific dataset (only with
				 * labels: code - label - locale) int headersFieldNo = dataStoreHeaders.getMetaData().getFieldCount(); //adds index informations to the metadata
				 * properties for(int i = 0; i < headersFieldNo; i++) { dataStoreHeaders.getMetaData().getFieldMeta(i).setProperty("index", i+1); } // gets the
				 * specific label using the code and the locale int posCode = (dataStoreHeaders.getMetaData().getFieldIndex("code") != -1) ?
				 * dataStoreHeaders.getMetaData().getFieldIndex("code") : dataStoreHeaders.getMetaData().getFieldIndex("CODE");
				 * 
				 * int posLabel = (dataStoreHeaders.getMetaData().getFieldIndex("label") != -1) ? dataStoreHeaders.getMetaData().getFieldIndex("label") :
				 * dataStoreHeaders.getMetaData().getFieldIndex("LABEL");
				 * 
				 * int posLocale = (dataStoreHeaders.getMetaData().getFieldIndex("locale") != -1) ? dataStoreHeaders.getMetaData().getFieldIndex("locale") :
				 * dataStoreHeaders.getMetaData().getFieldIndex("LOCALE");
				 * 
				 * List filterCodes = new ArrayList<String>(); List filterValues = new ArrayList<String>(); filterCodes.add(posCode);
				 * filterValues.add(fieldHeader); filterCodes.add(posLocale); filterValues.add(locale);
				 * 
				 * List headersFieldValues = (List)dataStoreHeaders.findRecords(filterCodes, filterValues); if (headersFieldValues != null &&
				 * headersFieldValues.size() > 0){ Record headerRec = (Record)headersFieldValues.get(0); IField headerField = null; if (headerRec != null){
				 * headerField = (IField) headerRec.getFieldAt(posLabel); } String label = (headerField != null) ? headerField.toString() : "";
				 * 
				 * if(label != null && !("").equalsIgnoreCase(label)){ fieldHeader = label; } } }else if (fieldHeaderType.equalsIgnoreCase("I18N")){ //gets the
				 * header value from the locale files
				 * 
				 * //EnginConf tmp = conf.getEngineConfig(); //System.out.println(tmp);
				 * 
				 * logger.debug("Export headers by locale file doesn't supported yet!"); } if (fieldHeader != null){
				 * 
				 * }
				 * 
				 * }
			...
			if (extractedFieldsMetaData != null && extractedFieldsMetaData.isEmpty()) {

				logger.debug("CreateExportFileAction: no metadata found for ColumnConfig. Headers will be created from original fields");

				for (int i = 0; i < extractedFields.size(); i++) {

					Field tempField = (Field) extractedFields.get(i);
					IFieldMetaData fFound = new FieldMetadata();
					fFound.setProperty("visible", Boolean.TRUE);
					fFound.setAlias(tempField.getName());
					fFound.setName(tempField.getName());
					fFound.setProperty("index", i);
					fFound.setType(Class.forName(tempField.getClassType().trim()));

					logger.debug("CreateExportFileAction: Created metadata for field [ " + fFound.getName() + " ]");

					extractedFieldsMetaData.add(fFound);

				}

				logger.debug("CreateExportFileAction: Headers created from original fields");
			}
			...
			if ("application/vnd.ms-excel".equalsIgnoreCase(mimeType)) {
				...
				String configLimit = conf.getProperty("EXPORT_ROWS_LIMIT");
				if (configLimit == null) {
					configLimit = "65000";
				}
				...
				logger.debug("A CSV document has to be written with " + csvDocument.getHeader().size() + " headers and " + csvDocument.getRows().size()
						+ " rows");
				file = File.createTempFile(docName, "." + fileExtension);
	...
	private String getFieldHeader(String datasetHeader, JSONArray jsonArray, IDataStore dataStore, IDataStore dataStoreHeaders, String locale)
			...
			while (it.hasNext()) {
				...
				if (key.equalsIgnoreCase(datasetHeader)) {
					...
					if (fieldHeaderType.equalsIgnoreCase("dataset")) {
						...
						int fieldValsize = dataStore.getFieldValues(posHeader).size();
						if (fieldValsize != 0) {
							fieldHeader = dataStore.getFieldValues(posHeader).get(0).toString();
						} else {
							fieldHeader = null;
						}
						...
						int posCode = (dataStoreHeaders.getMetaData().getFieldIndex("code") != -1) ? dataStoreHeaders.getMetaData().getFieldIndex("code")
						...
						List headersFieldValues = dataStoreHeaders.findRecords(filterCodes, filterValues);
						if (headersFieldValues != null && headersFieldValues.size() > 0) {
							...
							if (headerRec != null) {
								headerField = headerRec.getFieldAt(posLabel);
							}
				...
				/*
				 * if (fieldHeader != null){ Field headerF = new Field(fieldHeader, "java.lang.String", 100); extractedFields.add(headerF); for(int i = 0; i <
				 * fieldNo; i++) { IFieldMetaData fFound = dataStore.getMetaData().getFieldMeta(i); if(fFound.getName().equals(key)){
				 * fFound.setProperty("visible", Boolean.TRUE); fFound.setAlias(fieldHeader); fFound.setProperty("index", i);
				 * extractedFieldsMetaData.add(fFound); break; }
				 * 
				 * } }
				 */
	...
	private String getExportName(SourceBean request) throws JSONException {
		...
		/*
		 * JSONObject template = ((ConsoleEngineInstance)getEngineInstance()).getTemplate(); String st = (String)template.get("detailPanel");
		 */
