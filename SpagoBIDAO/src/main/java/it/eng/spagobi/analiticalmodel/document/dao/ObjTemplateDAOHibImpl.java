...
public class ObjTemplateDAOHibImpl extends AbstractHibernateDAO implements IObjTemplateDAO {
	...
	public void deleteBIObjectTemplate(Integer tempId) throws EMFInternalError {
		...
		try {
			...
			tx = aSession.beginTransaction();
			SbiObjTemplates hibObjTemp = (SbiObjTemplates) aSession.load(SbiObjTemplates.class, tempId);
			boolean wasActive = hibObjTemp.getActive();
			int documentId = hibObjTemp.getSbiObject().getBiobjId().intValue();
			SbiBinContents hibBinCont = hibObjTemp.getSbiBinContents();
			aSession.delete(hibBinCont);
			aSession.delete(hibObjTemp);

			if (wasActive) {
				// current active template is going to be deleted
				String hql = "from SbiObjTemplates sot where sot.sbiObject.biobjId = ? and sot.prog = "
						+ "(select max(t.prog) as maxprog from SbiObjTemplates t where t.sbiObject.biobjId = ?)";
				Query query = aSession.createQuery(hql);
				query.setInteger(0, documentId);
				query.setInteger(1, documentId);
				SbiObjTemplates previousTemplate = (SbiObjTemplates) query.uniqueResult();
				if (previousTemplate != null) {
					logger.debug("Setting previous template as active ...");
					previousTemplate.setActive(true);
					aSession.save(previousTemplate);
				} else {
					logger.debug("No previous template was found");
				}
			}
