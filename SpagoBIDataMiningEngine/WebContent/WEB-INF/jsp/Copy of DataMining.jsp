...
<%
	...
	executionOutput = (String)request.getAttribute("EXECUTION_OUTPUT");
	if(executionOutput == null){
		executionOutput ="";
	}
	doUploadDatasets = (Boolean)request.getAttribute("DO_UPLOAD_DATASETS");
	...
	<body>
    	...
    	<script type="text/javascript">  
	        ...
	        Ext.onReady(function(){
	    		...
	    		var uploadPanel = Ext.create('Sbi.datamining.UploadPanel',{datasets: <%=datasetsToUpload%>}); 
	        	var resultPanel = Ext.create('Sbi.datamining.ResultPanel',
						{ result : '<%=executionOutput%>', 
						  type: '<%=dataMiningEngineInstance.getOutputType()%>'},
						  items: [uploadPanel]); 
	    		var dataMiningPanel = Ext.create('Ext.panel.Panel',{items: [resultPanel, uploadPanel],
													    			style:"border: 1px solid red; height: 500px; width:500px;",layout: 'fit'});

	    		var dataMiningPanelViewport = Ext.create('Ext.container.Viewport', {
	    			layout:'fit',
	    			items: [dataMiningPanel]
	    	    });
	        });
	    </script>
