...
CREATE TABLE SBI_DATA_SOURCE (
		...
		WRITE_DEFAULT        TINYINT DEFAULT 0,
        USER_IN              VARCHAR(100) NOT NULL,
        USER_UP              VARCHAR(100),
        USER_DE              VARCHAR(100),
        TIME_IN              TIMESTAMP NOT NULL,
...
CREATE TABLE SBI_SNAPSHOTS (
       ...
       META_VERSION         VARCHAR(100),
       ORGANIZATION         VARCHAR(20), 
       CONTENT_TYPE VARCHAR(300) NULL DEFAULT NULL,
...
CREATE TABLE SBI_DIST_LIST (
       ...
       META_VERSION         VARCHAR(100),
       ORGANIZATION         VARCHAR(20),     
       CONSTRAINT XAK1SBI_SBI_DIST_LIST UNIQUE  (DL_ID, ORGANIZATION), 
       PRIMARY KEY (DL_ID)
) ;\p\g
...
CREATE TABLE SBI_DATA_SET (
       ...
       CONSTRAINT XAK1SBI_DATA_SET UNIQUE  (LABEL, ORGANIZATION),
       PRIMARY KEY (DS_ID)
) ;\p\g
...
CREATE TABLE SBI_DATA_SET_HISTORY (
	...
	VERSION_NUM	   		 INTEGER NOT NULL,
	OBJECT_TYPE   		 VARCHAR(50),
	DS_METADATA    		 NVARCHAR,
	PARAMS         		 VARCHAR(4000),
	CATEGORY_ID    		 INTEGER,
    FILE_NAME	  		 VARCHAR(300),
    QUERY   	  		 NVARCHAR,
    QUERY_SCRIPT   	  	 NVARCHAR DEFAULT NULL ,
    QUERY_SCRIPT_LANGUAGE	VARCHAR(100) DEFAULT NULL,
    DATA_SOURCE_ID		 INTEGER,
    ADRESS   	   		 VARCHAR(250),
	...
	META_VERSION         VARCHAR(100),	
	ORGANIZATION         VARCHAR(20),
	IS_PERSISTED  		 TINYINT DEFAULT 0,
	IS_FLAT_DATASET TINYINT DEFAULT 0,
...
CREATE TABLE SBI_OBJ_METADATA (
    ...
    CREATION_DATE 	    TIMESTAMP NOT NULL, 
       ...
       META_VERSION         VARCHAR(100),
       ORGANIZATION         VARCHAR(20),  
       CONSTRAINT XAK1SBI_OBJ_METADATA UNIQUE  (OBJ_META_ID, ORGANIZATION),
...
ALTER TABLE SBI_PARUSE ADD CONSTRAINT FK_SBI_PARUSE_3 FOREIGN KEY  ( DEFAULT_LOV_ID ) REFERENCES SBI_LOV ( LOV_ID ) ON DELETE RESTRICT;\p\g
...
ALTER TABLE SBI_KPI_COMMENTS ADD CONSTRAINT FK_SBI_KPI_COMMENT_1 FOREIGN KEY  ( BIN_ID ) REFERENCES SBI_BINARY_CONTENTS(BIN_ID);\p\g
ALTER TABLE SBI_KPI_COMMENTS ADD CONSTRAINT FK_SBI_KPI_COMMENT_2 FOREIGN KEY  ( KPI_INST_ID ) REFERENCES SBI_KPI_INSTANCE (ID_KPI_INSTANCE);\p\g
...
ALTER TABLE SBI_ORGANIZATION_ENGINE ADD CONSTRAINT FK_ENGINE_1 FOREIGN KEY (ENGINE_ID) REFERENCES SBI_ENGINES (ENGINE_ID);\p\g
ALTER TABLE SBI_ORGANIZATION_ENGINE ADD CONSTRAINT FK_ORGANIZATION_1 FOREIGN KEY (ORGANIZATION_ID) REFERENCES SBI_ORGANIZATIONS (ID);\p\g

ALTER TABLE SBI_ORGANIZATION_DATASOURCE ADD CONSTRAINT FK_DATASOURCE_2 FOREIGN KEY (DATASOURCE_ID) REFERENCES SBI_DATA_SOURCE (DS_ID);\p\g
ALTER TABLE SBI_ORGANIZATION_DATASOURCE ADD CONSTRAINT FK_ORGANIZATION_2 FOREIGN KEY (ORGANIZATION_ID) REFERENCES SBI_ORGANIZATIONS (ID);\p\g
...
ALTER TABLE SBI_META_MODELS ADD COLUMN MODEL_LOCKED BOOLEAN NULL DEFAULT NULL AFTER DESCR, ADD COLUMN MODEL_LOCKER VARCHAR(100) NULL DEFAULT NULL AFTER MODEL_LOCKED;
