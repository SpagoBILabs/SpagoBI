...
'{"Query":"' + ds_h.QUERY + '","queryScript":"' + NVL(DS_H.QUERY_SCRIPT,'') + '","queryScriptLanguage":"' + NVL(QUERY_SCRIPT_LANGUAGE,'') + '","dataSource":"' + NVL((SELECT LABEL FROM SBI_DATA_SOURCE WHERE DS_ID = DATA_SOURCE_ID),'') + '"}' 
...
'{"Script":"' + NVL(DS_H.SCRIPT,'') + '","scriptLanguage":"' + NVL(DS_H.LANGUAGE_SCRIPT,'') + '"}'
WHEN ds_h.OBJECT_TYPE = 'SbiCustomDataSet' THEN 
'{"customData":"' + NVL(DS_H.CUSTOM_DATA,'"{}"') + '","jClassName":"' + NVL(DS_H.JCLASS_NAME,'') + '"}'
WHEN ds_h.OBJECT_TYPE = 'SbiQbeDataSet' THEN 
'{"qbeDatamarts":"' + NVL(DS_H.DATAMARTS,'') + '","qbeDataSource":"' + NVL((SELECT LABEL FROM SBI_DATA_SOURCE WHERE DS_ID = DATA_SOURCE_ID),'') + '","qbeJSONQuery":"' + NVL(DS_H.JSON_QUERY,'') + '"}'
