 ...
 * Modifications by Davide Zerbetto on Jan 7th 2015: management of UTF-8 properties files.
 */
public class MessageBundle {
    ...
    /**
     * Retrieves the Locale of the current user checking for its language
     * profile in session.
    ...
    public static Locale getUserLocale() {
        ...
        String country = null;
        Object defaultLanguage = ConfigSingleton.getInstance().getAttribute(
        		DEFAULT_USER_LANGUAGE);
        Object defaultCountry = ConfigSingleton.getInstance().getAttribute(
        		DEFAULT_USER_COUNTRY);
        RequestContainer requestContainer = RequestContainer.getRequestContainer();
        if (requestContainer == null)
            TracerSingleton.log(Constants.NOME_MODULO, TracerSingleton.WARNING,
                "MessageBundle::getMessage: requestContainer nullo");
        else {
...
//	            	"MessageBundle::getMessage: country non specificato in common.xml imposta 'US'");
	...
	/**
	 * Questo metodo recupera una stringa individuata da un codice e da alcune proprietà dell'utente
	 * che ha richiesto il servizio (language,country).
	 * <p>
    ...
    public static String getMessage(String code, String bundle) {
        ...
        if (message == null) {
            TracerSingleton.log(Constants.NOME_MODULO, TracerSingleton.WARNING,
                    "MessageBundle::getMessage: codice [" + code + "] non trovato");
            message = code;
        }
        return message;
    ...
    /**
     * Retrieve a message in a hierarchical way: first it search in the path "properties.action.<action name>.code" or
     * "properties.page.<page name>.code", if the search fails it searches in the path "action.<action name>.code" or 
     * "page.<page name>.code" in the files enlisted in messages.xml, and if it fails again it searches in the path
     * "code" in the files enlisted in messages.xml.
