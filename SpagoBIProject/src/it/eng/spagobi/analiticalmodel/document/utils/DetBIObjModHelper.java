...
public class DetBIObjModHelper {
	...
	public BIObject recoverBIObjectDetails(String mod) throws Exception {
		...
		if (engineIdStr == null || engineIdStr.equals("")) {
			// if engine id is not specified take the first engine for the
			// biobject type
		...
		// TRY TO LOAD ALL THE FUNCTIONALITIES ASSOCIATED (into request) TO THE
		// BIOBEJCT
		...
		if (functionalitiesStr.size() == 0 && (communityFunctCode == null || communityFunctCode.equals(""))) {
			...
			// LowFunctionality commF =
			// DAOFactory.getLowFunctionalityDAO().loadLowFunctionalityByCode(communityFunctCode,
			// false);
			// functionalities.add(commF.getId());
			// }
		}
		// lOAD ALL THE FUNCTIONALITIES ASSOCIATED TO THE BIOBJECT (but not into
		// request)
		// First case: the current user is not an administrator (so he cannot
		// see all the functionalities)
		// and the modality is Modify. In this case some functionalities, that
		// the user cannot see, can be
		// already associated to the object (by different users). This
		// associations mustn't be erased.
		...
		// Second case: the current user is a local administrator (he can admin
		// only a part of the tree)
		// and the modality is Modify. In this case some funtionalities in oder
		// part of the tree, which the
		// user cannot see, can be already associated to the object. This
		// associations mustn't be erased.
	...
	public void fillResponse(String initialPath) throws EMFUserError {
		try {
			...
			// List languages =
			// ConfigSingleton.getInstance().getFilteredSourceBeanAttributeAsList("LANGUAGE_SUPPORTED",
			// "LANGUAGE", "language");
			...
			// response.setAttribute(DetailBIObjectModule.NAME_ATTR_LIST_LANGUAGES,
			// languages);
			List functionalities = new ArrayList();
			try {
