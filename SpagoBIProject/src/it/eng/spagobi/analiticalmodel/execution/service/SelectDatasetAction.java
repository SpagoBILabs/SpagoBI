 ...
 * 
 * 1) the worksheeet's edit service (WORKSHEET_WITH_DATASET_START_EDIT_ACTION) in 
...
//			Map<String, String> datasetParameterValuesMap = getDatasetParameterValuesMapFromString(parametersStrng);
			...
			Map<String, String> datasetParameterValuesMap = dataset.getParamsMap() != null ?
					dataset.getParamsMap() : new HashMap<String, String>();
	...
	protected Engine getEngine(String typeCode) {
		...
		try {
			...
			if (engines == null || engines.size() == 0) {
				...
				LogMF.warn(logger, "There are more than one engine for document of type ["+typeCode+"]. We will use the one whose label is equal to [{0}]", engine.getLabel());
			}
		} catch(Throwable t) {
