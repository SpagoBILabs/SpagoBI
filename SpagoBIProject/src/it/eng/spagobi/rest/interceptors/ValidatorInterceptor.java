...
/**
 * Notice this interceptor is implementing an the interface AcceptedByMethod. We also have to implement the method accept, where we receive the target method
 * information and we can decide whether if this interceptor will be executed or not. The org.jboss.resteasy.spi.interception.PreProcessInterceptor runs after a
 * JAX-RS resource method is found to invoke on, but before the actual invocation happens
 *
