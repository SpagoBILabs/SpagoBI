...
/**
 * Publishes the results of a list information request for dataSets
 * into the correct jsp page according to what contained into request. If Any errors occurred during the 
 * execution of the <code>ListDataSetModule</code> class, the publisher
 ...
 */
public class ListDataSetPublisher implements PublisherDispatcherIFace {
	static private Logger logger = Logger.getLogger(ListDataSetPublisher.class);
	...
	public String getPublisherName(RequestContainer requestContainer, ResponseContainer responseContainer) {
		...
		if(!errorHandler.isOK()) {
			if(GeneralUtilities.isErrorHandlerContainingOnlyValidationError(errorHandler)) {				
				logger.info("Publish: listDataSet"  );
				logger.debug("OUT");
				return "listDataSet";
			}
		}
