	...
	public void doService() {
		...
		try{
			...
			if(!(new File(dirS)).exists()){
				...
				throw new SpagoBIServiceException(SERVICE_NAME, "not existing directory "+dirS, null);
			}

			File zip = new File(filePath);
			if(!(zip.exists())){
				logger.error("not existing zip file "+filePath);
				throw new SpagoBIServiceException(SERVICE_NAME, "not existing zip file "+filePath, null);
			}
