			...
			for(int documentIndex = 0; documentIndex < documentLabels.length; documentIndex++) {
				...
				while (objectParametersIterator.hasNext()) {
					...
					if(executionController.directExecution()) {
						...
						if(isMailDispatch == true && isSnapshotDispatch==false && isFileSystemDispatch == false 
								&& isDistributionDispatch == false && isJavaClassDispatch == false && isFunctionalityTreeDispatch == false){
