...
public class AdapterHTTP extends HttpServlet {
	...
	private void handleSuspendResume(SourceBean serviceRequest, RequestContainer requestContainer) throws SourceBeanException {
		...
		if ((deleteSuspendResumeId != null) && (deleteSuspendResumeId.trim().length() > 0)) {
			...
			/*
			 * TracerSingleton.log(Constants.NOME_MODULO, TracerSingleton.INFORMATION, "AdapterHTTP::service: DELETE_SUSPEND_RESUME_ID Container to delete" +
			 * suspendedResumeContainers);
			 */
			if (suspendedResumeContainers != null) {
				aPermanentContainer.delAttribute(deleteSuspendResumeId);
			}
			/*
			 * suspendedResumeContainers = (SourceBean)aPermanentContainer.getAttribute(deleteSuspendResumeId); TracerSingleton.log(Constants.NOME_MODULO,
			 * TracerSingleton.INFORMATION, "AdapterHTTP::service: DELETE_SUSPEND_RESUME_ID Container to delete" + suspendedResumeContainers);
			 */
		}
		...
		if ((suspendResumeId != null) && (suspendResumeId.trim().length() > 0)) {
			TracerSingleton.log(Constants.NOME_MODULO, TracerSingleton.INFORMATION, "AdapterHTTP::service: SUSPEND_RESUME_ID [" + suspendResumeId
	...
	private void render(RequestContextIFace requestContext, Exception serviceException) throws Exception {
		...
		Boolean isHttpResponseFreezed = (Boolean) responseContainer.getAttribute(Constants.HTTP_RESPONSE_FREEZED);
