...
public class LightNavigationManager {
	...
	/**
	 * This method is responsible for the requests stack (the <code>LightNavigator</code>) modification. It controls if there is one of the following attributes
	 * in the service request: <code>LIGHT_NAVIGATOR_DESTROY</code> (requests stack is destroyed); <code>LIGHT_NAVIGATOR_RESET</code> (requests stack is
	 * resetted and current request is not put into stack); <code>LIGHT_NAVIGATOR_RESET_INSERT</code> (requests stack is resetted and current request is put
	 * into stack); <code>LIGHT_NAVIGATOR_DISABLED</code> (the navigator is disabled: the stack is not modified and the original request is returned);
	 ...
	 * <code>LIGHT_NAVIGATOR_REPLACE_LAST</code> (the more recent request is replaced by the request at input). If any errors occur, the original request is
	 * returned.
	 * 
	 * @param portletRequest
	 *            The <code>PortletRequest</code> object
	 * @param serviceRequest
	 *            The original service request <code>SourceBean</code>
	...
	public static SourceBean controlLightNavigation(Object request, SourceBean serviceRequest) throws Exception {
		TracerSingleton.log(Constants.NOME_MODULO, TracerSingleton.DEBUG,
				"Method controlNavigation in LightNavigationManager class invoked with service request:\n" + serviceRequest);
		...
		try {
			...
			if (lightNavigatorReset != null) {
				...
				TracerSingleton.log(Constants.NOME_MODULO, TracerSingleton.DEBUG, "LightNavigationManager: controlLightNavigation: " + "stack reset executed.");
			...
			TracerSingleton.log(Constants.NOME_MODULO, TracerSingleton.DEBUG, "LightNavigationManager: controlLightNavigation: "
					+ "putting the request in the stack " + (mark == null ? "without mark" : "with mark '" + mark + "'") + ". Returning unmodified request.");
			TracerSingleton.log(Constants.NOME_MODULO, TracerSingleton.DEBUG, "The requests stack now is:\n" + lightNavigator.toString());
			return serviceRequest;

		} catch (Exception e) {
			TracerSingleton.log(Constants.NOME_MODULO, TracerSingleton.MAJOR, "LightNavigationManager: " + "controlLightNavigation: an exception occurred. "
	 ...
	 *
	 * @param The
	 *            request object (it can be a <code>HttpServletRequest</code> or a <code>PortletRequest</code> object)
	 * @throws Exception
	 */
	private static LightNavigator retrieveLightNavigatorFromSession(Object request) throws Exception {
		if (!(request instanceof PortletRequest) && !(request instanceof HttpServletRequest)) {
			TracerSingleton.log(Constants.NOME_MODULO, TracerSingleton.CRITICAL, "LightNavigationManager: "
					+ "retrieveLightNavigatorFromSession: request object is neither a PortletRequest " + "nor an HttpServletRequest");
		...
		if (request instanceof PortletRequest) {
			...
			if (lightNavigatorObj != null) {
				...
				TracerSingleton.log(Constants.NOME_MODULO, TracerSingleton.DEBUG, "LightNavigationManager: "
						+ "retrieveLightNavigatorFromSession: LightNavigator not existing. Creating a new one and putting it " + "in session with key = ["
						+ sessionAttributeKey + "].");
	...
	private static void destroyLightNavigatorFromSession(Object request) throws Exception {
		if (!(request instanceof PortletRequest) && !(request instanceof HttpServletRequest)) {
			TracerSingleton.log(Constants.NOME_MODULO, TracerSingleton.CRITICAL, "LightNavigationManager: "
					+ "destroyLightNavigatorFromSession: request object is neither a PortletRequest " + "nor an HttpServletRequest");
			throw new Exception("request object is neither a PortletRequest nor an HttpServletRequest");
		}
