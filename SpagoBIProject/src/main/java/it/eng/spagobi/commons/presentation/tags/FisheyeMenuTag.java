...
public class FisheyeMenuTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			SpagoBITracer.critical(SpagoBIConstants.NAME_MODULE, this.getClass().getName(), "doStartTag", "Error while creating menu", e);
		...
		try {
			...
			SpagoBITracer.critical(SpagoBIConstants.NAME_MODULE, this.getClass().getName(), "doStartTag", "Impossible to send the stream");
			throw new JspException("Impossible to send the stream");
		}
		return SKIP_BODY;
	}

	private void makeMenu(StringBuffer htmlStream, StringBuffer jsStream, ConfigSingleton configuration, IEngUserProfile profile) throws EMFInternalError {
		...
		/*
		 * // create menu links based on user roles Collection userRoles = profile.getRoles(); Iterator userRolesIter = userRoles.iterator(); List views =
		 * configuration.getAttributeAsList("MENU.VIEW"); Iterator iterViews = views.iterator(); while(iterViews.hasNext()) { SourceBean viewSB =
		 * (SourceBean)iterViews.next(); String roleAssociated = (String)viewSB.getAttribute("role"); boolean isAllowed = false; while(userRolesIter.hasNext())
		 * { String userRole = (String)userRolesIter.next(); if(userRole.equalsIgnoreCase(roleAssociated)) { isAllowed = true; } } if(!isAllowed) { continue; }
		 * // user can see the view createViewMenuItem(htmlStream, jsStream, viewSB); }
		 */
	...
	/*
	 * public void createViewMenuItem(StringBuffer htmlStream, StringBuffer jsStream, SourceBean viewSB) { String width = (String)viewSB.getAttribute("width");
	 * String height = (String)viewSB.getAttribute("height"); String iconUrl = (String)viewSB.getAttribute("iconUrl"); String viewCode =
	 * (String)viewSB.getAttribute("code"); String viewTitle = (String)viewSB.getAttribute("title"); String link =
	 * "/spagobi/servlet/AdapterHTTP?PAGE=ViewBuilderPage&viewCode="+viewCode; createMenuItem(htmlStream, jsStream, viewCode, iconUrl, viewTitle, link, width,
	 * height, "false"); }
	 */

	private void createMenuItem(StringBuffer htmlStream, StringBuffer jsStream, String code, String iconUrl, String title, String link, String width,
		...
		String htmlFrame = "<div style='position:absolute;top:30px;left:30px;' id='loading" + code + "'>" + "<center>" + "<br/><br/>"
				+ "<span style='font-size:13pt;font-weight:bold;color:darkblue;'>" + "Loading" + "</span><br/><br/>" + "<img src='" + contexName
				+ "/img/wapp/loading.gif' />" + "</center>" + "</div>" + "<iframe id='frame" + code + "' " + "onload='parent.closeLoading" + code + "()' "
				+ "style='width:" + width + "px;height:" + height + "px;visibility:hidden;' " + "frameborder='0' scrolling='auto' noresize  src='" + link
				+ "' />";
		...
		jsStream.append("		win" + code + " = new Window('win" + code + "', {className: \"alphacube\", title: \"" + title + "\", resizable:" + resizable
				+ ", destroyOnClose:false, width:" + width + ", height:" + height + "});\n");
	...
	public int doEndTag() throws JspException {
		TracerSingleton.log(SpagoBIConstants.NAME_MODULE, TracerSingleton.INFORMATION, "FisheyeMenuTag::doEndTag:: invoked");
