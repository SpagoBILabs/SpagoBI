...
public class DocumentCompositionUtils {
	...
	public static String getExecutionUrl(String objLabel, SessionContainer sessionContainer, SourceBean requestSB) {
		...
		try {
			...
			/*
			 * ALL CONTROLS OF COMPATIBILITY ARE REMANDED TO THE SINGLE ENGINE CALLED // GET THE TYPE OF ENGINE (INTERNAL / EXTERNAL) AND THE SUITABLE BIOBJECT
			 * TYPES Domain engineType = null; Domain compatibleBiobjType = null; try { engineType =
			 * DAOFactory.getDomainDAO().loadDomainById(engine.getEngineTypeId()); compatibleBiobjType =
			 ...
			 * params.add(compatibleBiobjTypeCd); params.add(biobjTypeCd); //errorHandler.addError(new EMFUserError(EMFErrorSeverity.ERROR, 2002, params));
			 * return "2002|"; }
			 */
			...
			if ((className == null || className.trim().equals("")) && (document.getSnapshot() == null || !document.getSnapshot())) {
				// external engine
				// baseUrlReturn = obj.getEngine().getUrl() + "?";
				baseUrlReturn = obj.getEngine().getUrl();
				if (baseUrlReturn.indexOf("?") < 0)
					baseUrlReturn += "?";
