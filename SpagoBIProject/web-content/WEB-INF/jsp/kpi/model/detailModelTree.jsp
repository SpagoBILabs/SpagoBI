	...
	<%  
		...
		backUrlPars.put("MODULE", "ListModelTreeModule");
	...
	%>
		...
		<option value="<%=kpi.getKpiId()%>"
			...
			title="<%=StringEscapeUtils.escapeHtml(kpi.getComboBoxLabel(false))%>" <%=selected%>>
			<%=StringEscapeUtils.escapeHtml(kpi.getComboBoxLabel(true))%>
		</option>
		<%
		}
		%>
