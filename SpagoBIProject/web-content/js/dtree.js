...
dTree.prototype.node = function(node, nodeId) {
	...
	if (node.url) {
		...
		if (this.config.useSelection && ((node._hc && this.config.folderLinks) || !node._hc)) {
		    ...
		    str += ' onclick="javascript: ' + node.onclick + ';"';
		}
		str += '>';
	}
...
dTree.prototype.indent = function(node, nodeId) {
	...
	if (this.root.id != node.pid) {
		...
		if (node._hc) {
			str += '<a href="javascript: ' + this.obj + '.o(' + nodeId + ');"  onMouseOver="window.status=\' \';return true;" onFocus="window.status=\' \';return true;" onMouseOut="window.status=\' \';return true;"><img id="j' + this.obj + nodeId + '" src="';
...
dTree.prototype.getSelected = function() {
	var sn = this.getCookie('cs' + this.obj.substring(0,8));
	return (sn) ? sn : null;
};
...
dTree.prototype.s = function(id) {
	...
	if (this.selectedNode != id) {
		...
		if (this.config.useCookies) this.setCookie('cs' + this.obj.substring(0,8), cn.id);
	}
};
...
dTree.prototype.clearCookie = function() {
	...
	this.setCookie('co'+this.obj.substring(0,8), 'cookieValue', yesterday);
	this.setCookie('cs'+this.obj.substring(0,8), 'cookieValue', yesterday);
};
...
dTree.prototype.updateCookie = function() {
	...
	this.setCookie('co' + this.obj.substring(0,8), str);
};

// [Cookie] Checks if a node id is in a cookie
dTree.prototype.isOpen = function(id) {
	var aOpen = this.getCookie('co' + this.obj.substring(0,8)).split('.');
