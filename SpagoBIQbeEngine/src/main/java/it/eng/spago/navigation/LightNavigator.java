...
public class LightNavigator {
	 ...
	 *
	 * @param markedRequest
	 *            the <code>MarkedRequest</code> object to be added to the stack
	 */
	public void add(MarkedRequest markedRequest) throws NavigationException {
		if (markedRequest == null) {
			TracerSingleton.log(Constants.NOME_MODULO, TracerSingleton.CRITICAL, "LightNavigator: " + "add: the MarkedRequest object at input is null.");
	...
	/**
	 * Retrieves the <code>MarkedRequest</code> object at the desired position in the stack and deletes the more recent <code>MarkedRequest</code> objects
	...
	public MarkedRequest goBackToPosition(int i) throws NavigationException {
		...
		if (i < 0 || i > list.size() - 1) {
			TracerSingleton.log(Constants.NOME_MODULO, TracerSingleton.CRITICAL, "LightNavigator: " + "goBackToPosition: the position " + i
	...
	/**
	 * Deletes the more recent <code>MarkedRequest</code> object in the stack and adds the <code>MarkedRequest</code> passed at input in the first position of
	 * the stack.
	 *
	 * @param markedRequest
	 *            the <code>MarkedRequest</code> object that will replace the more recent <code>MarkedRequest</code> in the stack
	...
	public void replaceLast(MarkedRequest markedRequest) throws NavigationException {
		if (markedRequest == null) {
			TracerSingleton
					.log(Constants.NOME_MODULO, TracerSingleton.CRITICAL, "LightNavigator: " + "replaceLast: the MarkedRequest object at input is null.");
			throw new NavigationException("The MarkedRequest object at input is null.");
		}
		if (list.size() == 0) {
	...
	/**
	 * Retrieves the more recent <code>MarkedRequest</code> object in the stack with the mark passed at input and deletes the more recent
	 * <code>MarkedRequest</code> objects present in the stack.
	 *
	 * @param mark
	 *            The string mark of the desired <code>MarkedRequest</code> object in the stack.
	...
	public MarkedRequest goBackToMark(String mark) throws NavigationException {
		if (mark == null) {
			TracerSingleton.log(Constants.NOME_MODULO, TracerSingleton.MAJOR, "LightNavigator: " + "goBackToMark: the input mark is null.");
		...
		synchronized (list) {
			...
			if (toReturn == null) {
				TracerSingleton.log(Constants.NOME_MODULO, TracerSingleton.MAJOR, "LightNavigator: " + "goBackToMark: Request with mark '" + mark
						+ "' not found.");
				TracerSingleton.log(Constants.NOME_MODULO, TracerSingleton.DEBUG, "The requests stack is:\n" + this.toString());
