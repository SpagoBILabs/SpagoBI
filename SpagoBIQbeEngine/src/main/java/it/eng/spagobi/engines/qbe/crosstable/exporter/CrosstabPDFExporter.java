...
public class CrosstabPDFExporter {
	...
	private final Font cellFont = new Font(Font.HELVETICA, 8);
	 ...
	 *
	 * @param json
	 *            the JSON representation of the crosstab
	 * @param pdfDocument
	 *            the pdf document that should contains the crosstab
	 * @param numberFormat
	 ...
	 */
	public void export(JSONObject json, Document pdfDocument, DecimalFormat numberFormat) throws SerializationException, JSONException, BadElementException,
			DocumentException {
	...
	/**
	 * Builds the row headers. This method performs a depth first visit of the row headers tree
	 *
	 * @param siblings
	 *            : a level (L) of headers
	 ...
	 * @throws BadElementException
	 */
	private List<PdfPCell> buildColumnsHeader(JSONArray siblings, JSONArray rowHeadersDescription, int dataColumnNumber) throws JSONException,
			BadElementException {
		...
		for (int i = 0; i < columnNodes.size(); i++) {
			...
			JSONObject aNode = columnNodes.get(i);
			String text = (String) aNode.opt(Node.CROSSTAB_NODE_JSON_DESCRIPTION);
