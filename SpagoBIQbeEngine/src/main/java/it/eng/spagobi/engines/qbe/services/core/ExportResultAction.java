	...
	public void service(SourceBean request, SourceBean response) {
		...
		try {
			...
			if (!isFormEngineInstance) {
				...
				Assert.assertNotNull(getEngineInstance().getActiveQuery(), "Query object cannot be null in oder to execute " + this.getActionName()
						+ " service");
				...
				Assert.assertNotNull(responseType, "Input parameter [" + RESPONSE_TYPE + "] cannot be null in oder to execute " + this.getActionName()
						+ " service");
			...
			Assert.assertTrue(getEngineInstance().getActiveQuery().getSimpleSelectFields(true).size()
			...
			SourceBean config = ConfigSingleton.getInstance();
			...
			if (baseTemplateFileSB != null)
				baseTemplateFileStr = baseTemplateFileSB.getCharacters();
			File baseTemplateFile = null;
			if (baseTemplateFileStr != null)
				baseTemplateFile = new File(baseTemplateFileStr);
