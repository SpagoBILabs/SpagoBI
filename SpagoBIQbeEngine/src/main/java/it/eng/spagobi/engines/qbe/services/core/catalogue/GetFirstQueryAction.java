...
/**
 * This action is responsible to retrieve the first query contained into the catalog. It is used for Read-only users (a Read-only user is able only to execute
 * the first query of the catalog)
 *
...
public class GetFirstQueryAction extends AbstractQbeEngineAction {
	...
	public void service(SourceBean request, SourceBean response) {
		...
		try {
			...
			// serialize query
			JSONObject queryJSON = (JSONObject) SerializerFactory.getSerializer("application/json").serialize(query, getEngineInstance().getDataSource(),
					getLocale());
