		...
		for (int i = 0; i < selectedFields.size(); i++) {
			...
			if (f.isSimpleField()) {
				...
				try {
					...
					throw new SpagoBIEngineServiceException(getActionName(), "An error occurred while serializing field ["
							+ dataMartSelectField.getUniqueName() + " - " + dataMartSelectField.getAlias() + "]", e);
				}
