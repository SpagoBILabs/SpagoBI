...
public class QbeXMLTemplateParser implements IQbeTemplateParser {
	...
	private QbeTemplate parse(SourceBean template) {
		...
		try {
			...
			if (!TAG_ROOT_COMPOSITE.equalsIgnoreCase(templateName) && !TAG_ROOT_NORMAL.equalsIgnoreCase(templateName)
			...
			if (qbeTemplate.isComposite()) {
				...
				if (template.containsAttribute(TAG_DATAMART)) {
					...
					// Assert.assertUnreachable("Missing compolsury tag [" +
					// TAG_DATAMART + "]");
				// }
				...
				// worksheetSB = (SourceBean)
				// template.getAttribute(TAG_WORKSHEET_DEFINITION);
				// worksheetJSONTemplate = new
				// JSONObject(worksheetSB.getCharacters());
				// qbeTemplate.setProperty("worksheetJSONTemplate",
				// worksheetJSONTemplate);
				// } else {
				// logger.debug("Qbe template does not contain tag [" +
