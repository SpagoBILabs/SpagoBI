...
import it.eng.qbe.serialization.query.serializer.json.QuerySerializationConstants;
import it.eng.qbe.serialization.serializer.IDeserializer;
...
public class WorkSheetJSONDeserializer implements IDeserializer {
	...
	private void deserializeGlobalFilters(JSONObject workSheetDefinitionJSON, WorkSheetDefinition workSheetDefinition) throws Exception {
	...
	private void deserializeOptions(JSONObject workSheetDefinitionJSON, WorkSheetDefinition workSheetDefinition) throws Exception {
	...
	private SheetContent deserializeContent(JSONObject sheetJSON) throws Exception {
		...
		if (WorkSheetSerializationUtils.DESIGNER_PIVOT.equals(designer)) {
			toReturn = (CrosstabDefinition) SerializationManager.deserialize(content.getJSONObject(WorkSheetSerializationUtils.CROSSTABDEFINITION),
					"application/json", CrosstabDefinition.class);
	...
	private SheetContent deserializeChart(JSONObject content) throws JSONException, SerializationException {
	...
	private SheetContent deserializeTable(JSONObject content) throws JSONException, SerializationException {
		SheetContent toReturn;
