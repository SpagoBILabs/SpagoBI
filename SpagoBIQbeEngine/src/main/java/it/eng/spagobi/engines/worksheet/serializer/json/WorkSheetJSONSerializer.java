...
public class WorkSheetJSONSerializer implements ISerializer {
	...
	private JSONObject serializeContent(SheetContent content) throws SerializationException, JSONException {
		...
		if (content instanceof CrosstabDefinition) {
			JSONObject toReturn = new JSONObject();
			toReturn.put(WorkSheetSerializationUtils.CROSSTABDEFINITION, SerializationManager.serialize(content, "application/json"));
			boolean isStatic = ((CrosstabDefinition) content).isStatic();
			toReturn.put(WorkSheetSerializationUtils.DESIGNER, isStatic ? WorkSheetSerializationUtils.DESIGNER_STATIC_PIVOT
	 ...
	 *
	 * @param title
	 *            The JSONObject rapresentation of the header/footer
	 * @throws Exception
	 */
