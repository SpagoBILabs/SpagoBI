...
public class ExportWorksheetAction extends ExecuteWorksheetQueryAction {
	...
	public void service(SourceBean request, SourceBean response) {
		...
		try {
			...
			Assert.assertNotNull(engineInstance, "It's not possible to execute " + this.getActionName()
					+ " service before having properly created an instance of EngineInstance class: check document template");
			...
			if ("application/vnd.ms-excel".equalsIgnoreCase(mimeType)) {
				...
				throw new SpagoBIEngineException("Cannot export worksheet in " + mimeType
						+ " format, only application/vnd.ms-excel ans application/pdf are supported");
			}
	...
	public void exportToXLS(WorkSheetXLSExporter exporter, JSONObject worksheetJSON, JSONArray metadataPropertiesJSON, JSONArray parametersJSON,
	...
	public void exportMetadataToXLS(Workbook wb, WorkSheetXLSExporter exporter, CreationHelper createHelper, JSONArray metadataPropertiesJSON,
			JSONArray parametersJSON) throws Exception {
	...
	public void exportSheetToXLS(Workbook wb, JSONObject sheetJ, JSONArray fieldOptions, WorkSheetXLSExporter exporter, CreationHelper createHelper,
			WhereField splittingWhereField) throws Exception {
	...
	private int fillFiltersInfo(Map<String, List<String>> filters, Workbook wb, org.apache.poi.ss.usermodel.Sheet sheet, WorkSheetXLSExporter exporter,
			WhereField splittingWhereField, CreationHelper createHelper, int beginRowHeaderData, int beginColumnHeaderData) {
	...
	public int fillSheetContent(Workbook wb, org.apache.poi.ss.usermodel.Sheet sheet, JSONObject sheetJ, JSONArray fieldOptions,
			WhereField splittingWhereField, CreationHelper createHelper, WorkSheetXLSExporter exporter, Drawing patriarch, int sheetRow) throws Exception {
			...
			if (sheetType.equalsIgnoreCase(WorkSheetXLSExporter.CHART)) {
				...
				if (images != null) {
					...
					for (int j = 0; j < images.size(); j++) {
						...
						exporter.setImageIntoWorkSheet(wb, patriarch, images.get(j), CHART_START_COLUMN, CHART_END_COLUMN, sheetRow, CHART_HEIGHT_IN_ROWS,
				...
				JSONObject filters = sheetJ.optJSONObject(QbeEngineStaticVariables.FILTERS);
				CrossTab cs = getCrosstab(crosstabDefinitionJSON, fieldOptions, filters, sheetName, splittingWhereField, null);
	...
	/**
	 * Execute the query active in the engine instance and return the data store
	 * 
	 * @param splittingWhereField
	 *            The sheet's splitting filter, if any
	...
	public CrossTab getCrosstab(JSONObject crosstabDefinitionJSON, JSONArray fieldOptions, JSONObject optionalFilters, String sheetName,
			WhereField splittingWhereField, JSONArray calculateFieldsJSON) throws Exception {
		...
		if (!dataset.hasBehaviour(FilteringBehaviour.ID)) {
			/*
			 * If the dataset had the FilteringBehaviour, data was already filtered on domain values by the FilteringBehaviour itself. If the dataset hadn't the
			 * FilteringBehaviour, we must pust filters on domain values on query to temporary table
			 */
		...
		/*
		 * We must consider sheet filters anyway because temporary table contains data for all sheets, but different sheets could have different filters defined
		 * on them
		 */
		...
		CrosstabDefinition crosstabDefinition = (CrosstabDefinition) SerializationManager.deserialize(crosstabDefinitionJSON, "application/json",
