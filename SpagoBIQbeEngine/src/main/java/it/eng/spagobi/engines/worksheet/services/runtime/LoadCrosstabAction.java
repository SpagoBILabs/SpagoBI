...
public class LoadCrosstabAction extends AbstractWorksheetEngineAction {
	...
	public void service(SourceBean request, SourceBean response) {
		...
		try {
			...
			Assert.assertNotNull(crosstabDefinitionJSON, "Parameter [" + CROSSTAB_DEFINITION + "] cannot be null in oder to execute " + this.getActionName()
					+ " service");
			...
			if (!dataset.hasBehaviour(FilteringBehaviour.ID)) {
				/*
				 * If the dataset had the FilteringBehaviour, data was already filtered on domain values by the FilteringBehaviour itself. If the dataset hadn't
				 * the FilteringBehaviour, we must pust filters on domain values on query to temporary table
				 */
			...
			/*
			 * We must consider sheet filters anyway because temporary table contains data for all sheets, but different sheets could have different filters
			 * defined on them
			 */
