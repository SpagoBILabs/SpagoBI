 ...
 *
 *          The typical scenario: 1) the user fills the form 2) the user clicks the Open Worksheet button 3) the client send a message to the server, and the
 *          server starts the worksheet engine 4) the client sent to the server the form values and the server update the DataSet of the Worksheet engine with
 *          this new information.. This class performs the point 4
 *
 */
public class UpdateDataSetWithSmartFilterValues extends AbstractWorksheetEngineAction {
	...
	private Query updateQbeWithSmartFilterQuery(Query query, QbeEngineInstance qbeInstance, JSONObject jsonEncodedFormState) throws SerializationException {
		...
		if (jsonEncodedFormState != null) {
			...
			String store = ((JSONObject) SerializerFactory.getSerializer("application/json").serialize(query, qbeInstance.getDataSource(), getLocale()))
					.toString();
			...
			JSONObject template = qbeInstance.getFormState().getConf();
