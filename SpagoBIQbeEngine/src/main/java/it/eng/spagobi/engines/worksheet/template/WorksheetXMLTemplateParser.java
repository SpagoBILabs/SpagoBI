...
public class WorksheetXMLTemplateParser implements IWorksheetTemplateParser {
	...
	private WorksheetTemplate parse(SourceBean xml, Map env) {
		...
		try {
			...
			if (encodingFormatVersion.equalsIgnoreCase(CURRENT_VERSION)) {
				...
				template = worksheetViewerXMLTemplateLoader.load(xml);
				logger.debug("Encoding conversion has been executed succesfully");
			}

			// QBE block
			if (template.containsAttribute(TAG_QBE) || template.containsAttribute(TAG_QBE_COMPOSITE) || template.containsAttribute(TAG_SMART_FILTER)) {
	...
	public QbeEngineInstance startQbeEngine(SourceBean template, Map env) throws Exception {
		...
		if (qbeTemplate.getProperty("formJSONTemplate") != null && qbeTemplate.getProperty("formValuesJSONTemplate") != null) {
	...
	private Query applyFormViewerQueryTransformation(Query query, JSONObject template, JSONObject formValues, QbeEngineInstance qbeInstance) throws Exception {
		Query toReturn = null;
