	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		if ((session.isNew()) || (session.getAttribute("access_token") == null)) {
			if (((HttpServletRequest) request).getParameter("code") == null) {
				 ...
				 *  setting access token in request header 
				 */
				String url = oauth2Config.getProperty("REST_BASE_URL") + oauth2Config.getProperty("ROLES_PATH") + "?application_id=" + oauth2Config.getProperty("APPLICATION_ID");
