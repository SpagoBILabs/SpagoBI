...
public class TestJSONArray extends TestCase {
	 ...
	 */
	public static void _testJsonArray_ScintificNotation() {
		JSONArray jsonarray;
		String string;
	 ...
	 */
	public void _testConstructor_Nan() {
		try {
			jsonarray = new JSONArray();
	 ...
	 */
	public void _testConstructor_NegativeInfinity() {
		try {
			jsonarray = new JSONArray();
	 ...
	 */
	public void _testConstructor_PositiveInfinity() {
		try {
			jsonarray = new JSONArray();
	 ...
	 */
	public void _testPut_PositiveInfinity() {
		try {
			jsonarray = new JSONArray();
			jsonarray.put(Double.POSITIVE_INFINITY);
			fail("expecting JSONException here.");
	...
	public void testToString_Locations() {
		try {
			...
			jsonarray = new JSONArray(string);
			assertEquals("[\"San Francisco\",\"New York\",\"Seoul\",\"London\",\"Seattle\",\"Shanghai\"]", jsonarray.toString());
	...
	public void testConstructor_Collection() {
		...
		assertEquals("[\"string1\",\"string2\",\"string3\",\"string4\"]", jsonarray.toString());
	}
	...
	public void testConstructor_StringArray() {
		try {
			jsonarray = new JSONArray(new String[] { "string1", "string2" });
	...
	public void _testToString_Indents() {
		try {
			...
			jsonarray.put("abc");
			jsonarray.put(new JSONArray().put(new JSONArray()).put(new JSONArray().put("123").put("abc")));
			assertEquals(
					"[\n    \"123\",\n    {\"abc\": \"123\"},\n    \"abc\",\n    [\n        [],\n        [\n            \"123\",\n            \"abc\"\n        ]\n    ]\n]",
	 ...
	 */
	public void _testWriter_BadWriter() {
	...
	public void testPut_ObjectAndSpecificIndex() {
		try {
			...
			JSONObject b = new JSONObject("{b: 1}");
			JSONObject c = new JSONObject("{c: 1}");
			JSONObject d = new JSONObject("{d: 1}");
			jsonarray = new JSONArray();
			jsonarray.put(0, a);
			jsonarray.put(1, b);
			assertEquals("{\"a\":1}", jsonarray.get(0).toString());
			assertEquals("{\"b\":1}", jsonarray.get(1).toString());
			jsonarray.put(0, c);
			assertEquals("{\"c\":1}", jsonarray.get(0).toString());
			assertEquals("{\"b\":1}", jsonarray.get(1).toString());
			jsonarray.put(8, d);
			assertEquals("{\"d\":1}", jsonarray.get(8).toString());
			assertEquals(JSONObject.NULL, jsonarray.get(2));
			assertEquals(JSONObject.NULL, jsonarray.get(3));
			assertEquals(JSONObject.NULL, jsonarray.get(4));
			assertEquals(JSONObject.NULL, jsonarray.get(5));
