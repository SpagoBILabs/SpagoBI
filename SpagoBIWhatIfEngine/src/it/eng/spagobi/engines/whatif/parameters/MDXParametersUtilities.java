...
public class MDXParametersUtilities {
			...
			for (int i = 0; i < parameters.size(); i++) {
				...
				if (!(value instanceof String)) {
					...
					throw new SpagoBIEngineRuntimeException(
							"Parameter ["
									+ name
									+ "] contains more than one value, that is not admissible");
				}
				...
				toReturn = substituteParameterInMDXQuery(toReturn, alias,
						parameterValue);
			}
		}
		return toReturn;
	...
	public static String substituteParameterInMDXQuery(String query, String pname, String pvalue) {
		...
		while ((index = newQuery.indexOf("Parameter", ptr)) != -1) {
			...
			String thirdArg = newQuery.substring(ptr,
					newQuery.indexOf(",", ptr));
			// if the parameter type is STRING, add double apix to the value
			// passed by SpagoBI
			if (secondArg.equalsIgnoreCase("STRING")) {
				newQuery = newQuery.substring(0, ptr)
						+ '"'
						+ pvalue
						+ '"'
						+ newQuery.substring(newQuery.indexOf(",", ptr + 1),
								newQuery.length());
			} else {
				newQuery = newQuery.substring(0, ptr)
						+ pvalue
