...
public class HSQLDBEnviromentSingleton {
	...
	public synchronized void startDB() throws ClassNotFoundException, SQLException {
		...
		/*
		 * if (hsqlServer == null) { hsqlServer = new Server();
		 * 
		 * // HSQLDB prints out a lot of informations when // starting and closing, which we don't need now. // Normally you should point the setLogWriter // to
		 * some Writer object that could store the logs. hsqlServer.setLogWriter(null); hsqlServer.setSilent(true);
		 * 
		 * // The actual database will be named 'xdb' and its // settings and data will be stored in files // testdb.properties and testdb.script
		 ...
		 * 
		 * // Start the database! hsqlServer.start();
		 * 
		 * // We have here two 'try' blocks and two 'finally' // blocks because we have two things to close // after all - HSQLDB server and connection
		 * 
		 * // Getting a connection to the newly started database Class.forName("org.hsqldb.jdbcDriver"); }
		 */
	...
	public synchronized void closeDB() throws Exception {
		// Closing the server
		/*
		 * clean(); if (hsqlServer != null && !isLeaveOpen()) { hsqlServer.stop(); hsqlServer = null; }
		 */
	}
