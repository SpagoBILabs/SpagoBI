...
public class DocumentExecutionUtils {
	...
	public static List<DocumentParameters> getParameters(BIObject obj, String executionRole, Locale locale, String modality,HttpServletRequest req) {
		...
		if (parameters != null && parameters.size() > 0) {
			...
			while (it.hasNext()) {
				...
				// try {
				parametersForExecution.add(new DocumentParameters(parameter, executionRole, locale, document, req));
				// } catch (Exception e) {
	...
	public static JSONObject handleNormalExecution(UserProfile profile, BIObject obj, HttpServletRequest req, String env, String role, String modality,
		...
		DocumentUrlManager documentUrlManager = new DocumentUrlManager(getUserProfileFromSession(req,profile), locale);
		try {
			List errors = null;
	...
	public static HashMap<String, Object> getLovDefaultValues(String executionRole, BIObject biObject, BIObjectParameter objParameter, JSONObject requestVal,
		...
		try {
			IEngUserProfile profile = getUserProfileFromSession(req, UserProfileManager.getProfile());
	...
	public static String getLovResult(IEngUserProfile profile, ILovDetail lovDefinition, List<ObjParuse> dependencies, BIObject biObject,
		...
		if (lovDefinition instanceof QueryDetail) {
			// queries are cached
			String cacheKey = getCacheKey(getUserProfileFromSession(req, profile), lovDefinition, dependencies, biObject);
	...
	private static IEngUserProfile getUserProfileFromSession(HttpServletRequest req, IEngUserProfile engProfile) {
		if( engProfile == null){
