...
public class DocumentExecutionSnapshot extends AbstractSpagoBIResource {
	...
	public Response getSnapshots(@QueryParam("id") Integer biobjectId, @Context HttpServletRequest req) {
		...
		try {
			resultAsMap.put("schedulers", converetParameterValueObjTOJsonArr(snapshotsList,Snapshot.class));
	...
	private JSONArray converetParameterValueObjTOJsonArr(List valueList, Class<Snapshot> o){
		JSONArray jsonArr = new JSONArray();
		for(int i=0; i<valueList.size();i++){
			Snapshot op = (Snapshot) valueList.get(i);
			try {
	...
	public String getSnapshotContent(@QueryParam("biobjectId") Integer objectId, @QueryParam("idSnap") Integer idSnap, @Context HttpServletRequest req) {
		...
		IEngUserProfile profile = this.getUserProfileSession();
		Snapshot snap = null;
		BIObject obj;
		try {
