...
public class FolderResource extends AbstractSpagoBIResource {
	...
	@Path("/{label}")
	@Produces(MediaType.APPLICATION_JSON + "; charset=UTF-8")
	public Response getFolder(@PathParam("label") String label, @DefaultValue("false") @QueryParam("includeDocs") Boolean recoverBIObjects,
		...
		try {
			...
			LowFunctionality folder = dao.loadLowFunctionalityByCode(label, recoverBIObjects);
			if (folder == null)
				throw new SpagoBIRuntimeException("Document with label [" + label + "] doesn't exist");

			if (permissionOnFolder != null && !permissionOnFolder.isEmpty()) {
				if (!checkPermissionOnFolder(permissionOnFolder, folder, profile)) {
			...
			String jsonObject = JsonConverter.objectToJson(folder, folder.getClass());
