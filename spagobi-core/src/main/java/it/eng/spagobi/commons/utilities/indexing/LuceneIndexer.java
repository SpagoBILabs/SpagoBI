...
public class LuceneIndexer {
	...
	public static void addBiobjToIndex(BIObject biObj) {
		...
		try {
			...
			writer = new IndexWriter(FSDirectory.open(new File(index)), new StandardAnalyzer(Version.LUCENE_CURRENT), false,
					new IndexWriter.MaxFieldLength(1000000));
			Document doc = new Document();
			String uid = createUidDocument(null, String.valueOf(biObj.getId().intValue()));
			doc.add(new Field(IndexingConstants.UID, uid, Field.Store.YES, Field.Index.NOT_ANALYZED));
			doc.add(new Field(IndexingConstants.BIOBJ_ID, String.valueOf(biObj.getId().intValue()), Field.Store.YES, Field.Index.NOT_ANALYZED));
			doc.add(new Field(IndexingConstants.BIOBJ_NAME, biObj.getName(), Field.Store.NO, Field.Index.ANALYZED));
			if (biObj.getDescription() != null) {
				doc.add(new Field(IndexingConstants.BIOBJ_DESCR, biObj.getDescription(), Field.Store.NO, Field.Index.ANALYZED));
			}
			doc.add(new Field(IndexingConstants.BIOBJ_LABEL, biObj.getLabel(), Field.Store.NO, Field.Index.ANALYZED));

			doc.add(new Field(IndexingConstants.TENANT, getTenant(biObj), Field.Store.YES, Field.Index.NOT_ANALYZED));
			...
			logger.info("Indexing time:: " + (end.getTime() - start.getTime()) + " milliseconds");
			logger.debug("OUT");
		...
		try {
			...
			if (metadata != null && !metadata.isEmpty()) {
				...
				while (it.hasNext()) {
					...
					ObjMetacontent objMetacontent = DAOFactory.getObjMetacontentDAO().loadObjMetacontent(objMetadata.getObjMetaId(), biObj.getId(), null);
			...
			if (writer != null) {
				try {
					...
					logger.error("Error in writing indexer", e);
				}
			}
			logger.error("Error in writing indexer", e);
		} finally {
	...
	/**
	 * Method which indexes metadata binary contents. The logic is: if uid exists do not add to Document otherwise add it. Binary contents can be either html or
	 * simple text.
	 *
	 * @throws Exception
	 */
			...
			if (biobjects != null) {
				for (int k = 0; k < biobjects.size(); k++) {
					...
					if (!hasMetacontent) {
						...
						doc.add(new Field(IndexingConstants.UID, uid, Field.Store.YES, Field.Index.NOT_ANALYZED));
						// biobjid = uid
						doc.add(new Field(IndexingConstants.BIOBJ_ID, uid, Field.Store.YES, Field.Index.NOT_ANALYZED));
						doc.add(new Field(IndexingConstants.BIOBJ_NAME, biobjects.get(k).getName(), Field.Store.NO, Field.Index.ANALYZED));
						if (biobjects.get(k).getDescription() != null) {
							doc.add(new Field(IndexingConstants.BIOBJ_DESCR, biobjects.get(k).getDescription(), Field.Store.NO, Field.Index.ANALYZED));
						}
						doc.add(new Field(IndexingConstants.BIOBJ_LABEL, biobjects.get(k).getLabel(), Field.Store.NO, Field.Index.ANALYZED));
						doc.add(new Field(IndexingConstants.TENANT, getTenant(biobjects.get(k)), Field.Store.YES, Field.Index.NOT_ANALYZED));
	...
	private static void addFieldsToDocument(Document doc, String binId, Integer biobjId, String metaName, Domain domain, String htmlContent, byte[] content)
			throws UnsupportedEncodingException {
		logger.debug("IN");
		String uid = createUidDocument(binId, String.valueOf(biobjId.intValue()));
		doc.add(new Field(IndexingConstants.UID, uid, Field.Store.YES, Field.Index.NOT_ANALYZED));
		doc.add(new Field(IndexingConstants.BIOBJ_ID, String.valueOf(biobjId.intValue()), Field.Store.YES, Field.Index.NOT_ANALYZED));
		if (metaName != null) {
			doc.add(new Field(IndexingConstants.METADATA, metaName, Field.Store.YES, Field.Index.NOT_ANALYZED));
		}
		if (domain.getValueCd().equalsIgnoreCase(LONG_TEXT)) { // index
																...
																// content
			if (htmlContent != null) {
				doc.add(new Field(IndexingConstants.CONTENTS, htmlContent, Field.Store.NO, Field.Index.ANALYZED));
				logger.info("adding html binary content " + doc.get(IndexingConstants.UID));
				logger.info("-> " + htmlContent);
			}
		} else if (domain.getValueCd().equalsIgnoreCase(SHORT_TEXT)) {// index simple text binary
																		// content
			if (content != null) {
				doc.add(new Field(IndexingConstants.CONTENTS, new String(content, "UTF-8"), Field.Store.NO, Field.Index.ANALYZED));
			...
			if (subobjects != null) {
				for (int i = 0; i < subobjects.size(); i++) {
					...
					if (subObj.getName() != null) {
						doc.add(new Field(IndexingConstants.SUBOBJ_NAME, subObj.getName(), Field.Store.YES, Field.Index.ANALYZED));
					}
					if (subObj.getDescription() != null) {
						doc.add(new Field(IndexingConstants.SUBOBJ_DESCR, subObj.getDescription(), Field.Store.YES, Field.Index.ANALYZED));
					}
				}
			}
		} catch (EMFUserError e) {
